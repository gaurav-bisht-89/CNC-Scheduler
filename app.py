...
#SHARING LINK HERE
# =====================================================================
# Replace this placeholder link with your actual Google Sheets sharing link
GOOGLE_SHEET_URL = "https://docs.google.com/spreadsheets/d/1iuFMQHJssHz4z0_zW-HQ6gMTAnQiRiqB6m2_hboiOFc/edit?gid=0#gid=0"

def get_sheet_csv_url(base_url, sheet_name):
    # Splits the standard sharing link to pull raw CSV data from specific tabs
    sub_parts = base_url.split('/edit')
    return f"{sub_parts[0]}/gviz/tq?tqx=out:csv&sheet={sheet_name}"

@st.cache_data(ttl=15)  # The webpage checks for updates every 15 seconds
def load_cloud_scheduler_engine(base_url):
    orders = pd.read_csv(get_sheet_csv_url(base_url, "Orders"))
    routing = pd.read_csv(get_sheet_csv_url(base_url, "RoutingMaster"))
    machines = pd.read_csv(get_sheet_csv_url(base_url, "MachineMaster"))
    calendar = pd.read_csv(get_sheet_csv_url(base_url, "WorkingCalendar"))
    maintenance = pd.read_csv(get_sheet_csv_url(base_url, "Maintenance"))

    for df in [orders, routing, machines, calendar, maintenance]:
...
# Run Live Interface Assembly
try:
    schedule_df, capacity_matrix, baseline_capacities, total_available_shop_minutes, shop_dates, master_part_list, master_machine_list, machines_master, orders_processing = load_cloud_scheduler_engine(GOOGLE_SHEET_URL)
except Exception as e:
    st.error("⚠️ Cloud connection error. Please verify your Google Sheets sharing link settings.")
    st.stop()

st.title("🏭 CNC SHOP FLOOR OPERATIONS CONTROL CENTER")
selected_view = st.sidebar.radio("View Mode:", ["📊 Machine Load Projections", "📦 Component Tracking", "📋 Executive Reports"])
deadline_1, deadline_2 = pd.to_datetime('2026-06-25'), pd.to_datetime('2026-07-05')

if selected_view == "📊 Machine Load Projections":
    st.subheader("📊 Post-Adjustment Machine Load Distribution")
    balanced_load_data = []
    for m_id in master_machine_list:
        ...
        m_name = machines_master[machines_master['Machine ID'] == m_id]['Machine Name'].values[0] if m_id in machines_master['Machine ID'].values else "Auxiliary Channel"
        balanced_load_data.append({'Machine ID': m_id, 'Utilization (%)': round(true_util, 1)})
    
    fig_load = px.bar(pd.DataFrame(balanced_load_data), x='Machine ID', y='Utilization (%)', text='Utilization (%)', color='Utilization (%)', color_continuous_scale='YlOrRd')
    fig_load.add_hline(y=100.0, line_dash="dash", line_color="red")
    st.plotly_chart(fig_load, use_container_width=True)

elif selected_view == "📦 Component Tracking":
    st.subheader("📦 Part-Wise Logistics Progress Heatmap")
    ...
    for idx, p_no in enumerate(master_part_list):
        ...
        for jdx, dt in enumerate(shop_dates):
            if dt.weekday() == 6:
                ...
                hover_text_part[idx, jdx] = "Sunday Shutdown"
                continue
            ...
            hover_text_part[idx, jdx] = f"Part: {p_no}<br>Yield Level: {finished:.1f} / {target_qty} pcs"
    ...
    st.subheader("📋 Executive Shipping Summary Tables")
    ...
    for p in master_part_list:
        ...
        m1_summary.append({'Part ID': p, 'Part Name': p_name, 'Target': f"{t1} pcs", 'Scheduled Yield': f"{int(round(comp1))} pcs", 'Shortfall': f"{max(0, t1-int(round(comp1)))} pcs"})
        ...
        m2_summary.append({'Part ID': p, 'Part Name': p_name, 'Target': f"{t2} pcs", 'Scheduled Yield': f"{int(round(comp2))} pcs", 'Shortfall': f"{max(0, t2-int(round(comp2)))} pcs"})

    st.markdown("### 🔵 June 25th Milestone Deliveries")
    st.dataframe(pd.DataFrame(m1_summary), use_container_width=True, hide_index=True)
    st.markdown("### 🟢 July 5th Milestone Deliveries")
